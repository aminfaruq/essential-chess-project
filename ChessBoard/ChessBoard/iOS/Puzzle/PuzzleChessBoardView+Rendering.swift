//
//  PuzzleChessBoardView+Rendering.swift
//  NativeChessBoard
//
...
extension PuzzleChessBoardView {
    ...
    func renderPieces() {
        ...
        for rank in 1...8 {
            for file in ["a", "b", "c", "d", "e", "f", "g", "h"] {
                ...
                if puzzleMode == .learnThePieces && piece.color != userColor {
                    if let image = UIImage(systemName: "star.fill")?.withTintColor(.systemYellow, renderingMode: .alwaysOriginal) {
                        let imageView = UIImageView(image: image)
                        imageView.contentMode = .scaleAspectFit
                        // Give it a strong, dark drop shadow to improve contrast on light squares
                        ...
                        imageView.snp.makeConstraints { make in make.edges.equalToSuperview().inset(10) }
                        pieceImageViews[squareString] = imageView
                    }
                } else if let image = UIImage(named: "\(currentPieceTheme)_\(colorLetter)\(pieceLetter)") {
                    let imageView = UIImageView(image: image)
                    imageView.contentMode = .scaleAspectFit
                    squareView.addSubview(imageView)
    ...
    func isPromotionMove(from source: String, to target: String) -> Bool {
        ...
        // Ensure the move is actually legal before considering it a promotion
    ...
    func drawNativeRedArrow(from sourceStr: String, to targetStr: String) {
        ...
        arrowLayer.strokeColor = UIColor.systemRed.withAlphaComponent(0.8).cgColor
        ...
        headLayer.fillColor = UIColor.systemRed.withAlphaComponent(0.8).cgColor
    ...
    func clearSolutionArrow() {
        solutionArrowContainer?.removeFromSuperview()
        solutionArrowContainer = nil
    }

    func drawGreenSolutionArrow(from sourceStr: String, to targetStr: String) {
        clearSolutionArrow()
        ...
        let arrowLayer = CAShapeLayer()
        arrowLayer.path = path.cgPath
        arrowLayer.strokeColor = UIColor.systemGreen.withAlphaComponent(0.8).cgColor
        arrowLayer.lineWidth = 6
        arrowLayer.lineCap = .round
        ...
        let headLayer = CAShapeLayer()
        headLayer.path = headPath.cgPath
        headLayer.fillColor = UIColor.systemGreen.withAlphaComponent(0.8).cgColor

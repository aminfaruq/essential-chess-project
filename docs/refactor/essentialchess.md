...
Progress: **7 / 12 sub-phases** done. Test suite green (143 tests, `platform=macOS`); app
build verified with `CODE_SIGNING_ALLOWED=NO`.
...
- Phase 3 — Presentation hygiene (`mvvm-swiftui`)
  - [ ] 3a: adapters `@MainActor`, `DispatchQueue.main` hops removed
...
#### 3a — @MainActor adapters — pending
- `LanguageAdapter`, `ThemeAdapter`, `ProgressAdapter`, `UserNotificationsAdapter`:
  annotate `@MainActor` on the class (or the emitting methods) and delete the seven
  `DispatchQueue.main.async` hops.
- Verify SwiftUI call sites (all main-thread) compile; run UI-VM tests.
